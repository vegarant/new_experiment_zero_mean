...
automap_gnoise_levels_rmse_vec = np.load(join(src_data,'NMARESP_automap_rmse_gnoise_levels_row3.npy'))

automap_gnoise_outputs = np.load(join(src_data,'NMARESP_automap_output_gnoise_levels_row3.npy'))

automap_gnoise_outputs = np.reshape(automap_gnoise_outputs,(automap_gnoise_outputs.shape[0],128,128))

lasso_gnoise_outputs = np.load(join(src_data,'NMARESP_lasso_gnoise_recons_row3.npy'))
...
data = scipy.io.loadmat(join(src_data, f'new_image.mat'));
mri_data = data['im'];
...
plt.legend(['AUTOMAP','LASSO 100 iter','LASSO 250 iter','LASSO 500 iter','LASSO 1000 iter'],loc='upper left', frameon=False, fontsize=fsize-5)

#plt.ylim([0.012,0.024])
plt.ylabel('RMSE', fontsize=fsize)
plt.xlabel('$||e||_2 / ||y||_2$', fontsize=fsize)
plt.xticks(fontsize=fsize-5)
plt.yticks(fontsize=fsize-5)

plt.savefig('figure6a1_row3.pdf')
...
plt.legend(['AUTOMAP','LASSO 100 iter','LASSO 250 iter','LASSO 500 iter','LASSO 1000 iter'],loc='lower left', frameon=False, fontsize=fsize-5)


plt.ylabel('SSIM', fontsize=fsize)
plt.xlabel('$||e||_2 / ||y||_2$', fontsize=fsize)
plt.xticks(fontsize=fsize-5)
plt.yticks(fontsize=fsize-5)

plt.savefig('figure6b1_row3.pdf')
