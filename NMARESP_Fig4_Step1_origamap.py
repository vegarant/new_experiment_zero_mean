...
automap_recon_onlygauss_arr = np.zeros(shape=(5,len(im_nbrs),128,128))
...
for r_value in range(0,5):
    ...
    for i in range(len(im_nbrs)):
        ...
        onlygauss_input = (Ax+np.random.normal(0,scale=0.01,size=Ax.shape))*input_scale
        output_onlygauss = raw_f(onlygauss_input)
        
        # print(output.shape)

        # automap_recon_advonly_arr[r_value,i,:,:] = output_advonly
        automap_recon_onlygauss_arr[r_value,i,:,:] = output_onlygauss
...
np.save(join(dest_data, fname_out_onlygauss), automap_recon_onlygauss_arr)
