...
def pertub_SGA(f, dQ, batch, epoch, r0, v0, momentum=0.9, lr=0.01, verbose=True,
               max_r_norm=float('Inf'), max_diff_norm=float('Inf'), max_r_over_x_norm=float('Inf')):
    ...
    :param max_r_over_x_norm: Stop iterations if ||r||_{2}/||x||_{2} > max_r_over_x_norm.
    ...
    if (verbose):
        ...
        print('max_r_over_x_norm: %g' % max_r_over_x_norm);
        print('------------------------------------');
    ...
    r = r0;
    v = v0;
    while (i <= epoch and norm_fx_fxr < max_diff_norm and norm_r < max_r_norm and norm_r_over_x < max_r_over_x_norm):
        ...
        norm_r_over_x = norm_r/norm_x0
        
        next_str = \
        '%2d: |f(x)-f(x+r)|: %8g, |r|: %8g, |f(x)-f(x+r)|/|r| : %8g, |r|/|x|: %8g, |x|: %8g' \
        % (i, norm_fx_fxr, norm_r, norm_fx_fxr/norm_r, norm_r/norm_x0, norm_x0);
