...
# import _2fc_2cnv_1dcv_2GPU_REALIMAG_nozmean_tg_upg_CFLOAT_L1_3_3_3_mnoise as arch
import matplotlib.image as mpimg;
import matplotlib.pyplot as plt;
import numpy as np;
from .automap_config import src_weights, src_data, \
...
def sample_image(im, k_mask_idx1, k_mask_idx2):
    ...
    for i in range(batch_size):
        ...
        # noise = np.random.normal(loc=0, scale=0.01, size=single_im.shape)
        # print(noise)
        
        # single_im = single_im + noise
...
def RESP_hand_f(raw_f, x_im, noise_im,k_mask_idx1, k_mask_idx2):
    """
    Takes in an image, subsample it in k-space, and reconstructs it using the 
    AUTOMAP network.
    ...
    im_ksp = sample_image(x_im, k_mask_idx1, k_mask_idx2);
    noise_ksp = sample_image(noise_im, k_mask_idx1, k_mask_idx2);

    im_rec = raw_f(im_ksp+noise_ksp);
...
def compile_network(sess, batch_size, fname_weights, src_weights=src_weights):
    
    f_weights = read_automap_weights(src_weights, fname_weights)
    
    print('finished reading weights')

    print(f_weights['W1_cnv'])
    print('finished test')
    ...
    tf_ycrop = net['ycrop'];
    tf_x = net['x'];
    tf_label = tf.compat.v1.placeholder(tf.float32);
